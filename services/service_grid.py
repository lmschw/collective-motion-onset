...
def get_adjacent_cross_cells_distance_one(cell_idx, grid_size):
    ...
    if is_top_row(cell_idx=cell_idx):
    ...
    if is_bottom_row(cell_idx=cell_idx, grid_size=grid_size):
        bottom = cell_idx % grid_size[0] - grid_size[0] + 1
    else:
        bottom = cell_idx + 1
    ...
    if is_first_column(cell_idx=cell_idx, grid_size=grid_size):
        left = cell_idx + (grid_size[1] * (grid_size[0]-1))
    else:
...
def get_adjacent_square_cells_eight(cell_idx, grid_size):
    ...
    if is_top_row(cell_idx=cell_idx, grid_size=grid_size) and is_last_column(cell_idx=cell_idx, grid_size=grid_size):
        top_right = grid_size[0] - 1
    # top right for the top row is the last cell on the next column
    elif is_top_row(cell_idx=cell_idx, grid_size=grid_size):
        top_right = cell_idx + (2 * grid_size[0]) - 1
    # top right for the last column is the first cell in the row above
    elif is_last_column(cell_idx=cell_idx, grid_size=grid_size):
        top_right = cell_idx % grid_size[0]
    # for all others
    else:
        top_right = cell_idx + grid_size[0] - 1

    # bottom right for the bottom right corner is the top left cell
    if is_bottom_row(cell_idx=cell_idx, grid_size=grid_size) and is_last_column(cell_idx=cell_idx, grid_size=grid_size):
        bottom_right = 0
    # bottom right for the bottom row is the top cell in the next column, so the next cell
    elif is_bottom_row(cell_idx=cell_idx, grid_size=grid_size):
    ...
    if is_bottom_row(cell_idx=cell_idx, grid_size=grid_size) and is_first_column(cell_idx=cell_idx, grid_size=grid_size):
        bottom_left = grid_size[0] + (grid_size[1]-1)
    # bottom left for the bottom row is the first cell in the previous column
    elif is_bottom_row(cell_idx=cell_idx, grid_size=grid_size):
        bottom_left = cell_idx - grid_size[0]
    # bottom left or the first column is the cell on the next row in the last column
    elif is_first_column(cell_idx=cell_idx, grid_size=grid_size):
    ...
    if is_top_row(cell_idx=cell_idx, grid_size=grid_size) and is_first_column(cell_idx=cell_idx, grid_size=grid_size):
