...
def save_model(simulation_data, path="sample.json", model_params=None, saveInterval=1):
    ...
    """
    agents, placements, grids = simulation_data
    times = list(range(len(agents)))
    dict = {"time": __get_specified_intervals(saveInterval, times), 
            "agents": __get_specified_intervals(saveInterval, agents), 
            "grids": __get_specified_intervals(saveInterval, grids),
            "placements": __get_specified_intervals(saveInterval, placements)}
...
def load_model(path):
    ...
    times = np.array(loadedJson["times"])
    agents = np.array(loadedJson["agents"])
    grids = np.array(loadedJson["grids"])
    placements = np.array(loadedJson["placements"])
    return model_params, times, (agents, grids, placements)
...
def load_models(paths):
    ...
    params = []
    for path in paths:
        model_params, model_times, simulation_data = load_model(path)
